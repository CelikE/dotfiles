...
# ai
alias cdcomp='cd ~/workspace/ai/complementary-products'

# Workspace change directory
alias cdmc='cd ~/workspace/next/mc'
alias cddel='cd ~/workspace/next/delivery'
alias cdback='cd ~/workspace/next/mc'
...
alias java19='export JAVA_HOME=/usr/lib/jvm/java-19-openjdk'

#git pr hashtag
alias gitpr='git_tag=$(git tag --list "v*" --sort=-v:refname | grep -v "pre" | head -n 1) && echo "Latest non-pre-release tag: $git_tag" && git log $git_tag..HEAD --merges --pretty=format:"%s%n%b" | grep -oP "(?<=#)\d+" | sort -u'
...
#header
alias gitlr='git log --oneline $1..HEAD'
...
__conda_setup="$('/home/celik/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/celik/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/celik/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/celik/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
# Source function script for 'venv' command
. /usr/local/share/venv/venv
fpath+=( /usr/local/share/venv )
autoload -Uz venv
